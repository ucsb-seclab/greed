    ...
    def handle(self, state:SymbolicEVMState):
        ...
        if concrete(self.size_val):
            ...
            for i in range(succ.MAX_CALLDATA_SIZE):
                succ.memory[self.destOffset_val + i] = z3.If(self.size_val < i, succ.memory[self.destOffset_val + i], succ.calldata[self.calldataOffset_val + i])
...
class TAC_Blockhash(TAC_Statement):
    ...
    def handle(self, state:SymbolicEVMState):
        ...
        succ.registers[self.res1_var] = utils.ctx_or_symbolic('BLOCKHASH[%d]' % self.blockNumber_val, succ.ctx, succ.xid)
