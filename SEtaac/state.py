    ...
    def jumpi_handler(self, s0, s1):
        ...
        if concrete(s1):
            ...
            if self.pc + 1 == next_target:
                # next addr in path is the fallthrough branch (false)
                if not (concrete(s0) and s0 == next_target):
                    self.constraints.append(s1 == 0)
            elif concrete(s0) and s0 == next_target:
                # next addr in path is the true branch
                if self.pc + 1 != next_target:
                    self.constraints.append(s1 != 0)
                self.pc = s0
                if self.pc >= len(self.code) or not self.program[self.pc].name == 'JUMPDEST':
                ...
                # jump target is symbolic
                raise SymbolicError('Symbolic jump target')
            else:
                # both paths are possible and the target path doesn't help in choosing one or the other
                # ideally, we would fork here
                # instead, since that is not currently supported, let's check if both branches are sat
                s = get_solver()
                s.add(self.constraints)
                sat_true = is_true(s1 == 1, s)
                sat_false = is_true(s1 == 0, s)
