...
def main(args):
    ...
    if args.calldata is not None:
        ...
        log.info(f"Adding constraints for function id: {args.function_id}")
        function_id_int = int(args.function_id, 16)
        function_id_bytes = function_id_int.to_bytes(length=(function_id_int.bit_length() + 7) // 8, byteorder='big')
        for b in enumerate(function_id_bytes):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--function-id", type=str, action="store", help="Concrete function identifier (in hex)")
