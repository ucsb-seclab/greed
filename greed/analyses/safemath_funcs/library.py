...
from .types import SafeMathFunc, SafeMathFuncReport
...
def identify_library_safemath_func(
    project: "Project", function: "TAC_Function"
) -> Optional[SafeMathFuncReport]:
    ...
    if len(return_stmts_and_vars_no_constant_returns) != 1:
        log.debug(
            f"Function {function.name} has multiple return blocks with non-constant returns"
        )
        return None
    ...
    with _with_timeout(10.0):
        with _enable_lazy_solves():
            ...
            while True:
                ...
                simgr.move(
                    from_stash="found",
                    to_stash="pruned",
                    filter_func=lambda s: s.solver.is_unsat(),
                )
                ...
                simgr.move(
                    from_stash="found",
                    to_stash="pruned",
                    filter_func=is_returning_constant,
                )
        ...
        log.debug(
            f"Function {function.name} reached the return statement at state {found_state}"
        )
        ...
        if found_state.solver.are_formulas_sat(guess_assumptions):
            ...
            if overflow_possible:
                log.debug(
                    f"Function {function.name} addition can overflow without revert"
                )
                return None
        ...
        if found_state.solver.are_formulas_sat(guess_assumptions):
            ...
            minuend, subtrahend = (
                (operand_0, operand_1) if sub_forward else (operand_1, operand_0)
            )
            ...
            sub_underflow_possible = found_state.solver.are_formulas_sat(
                sub_underflow_assumption
            )
        ...
        if found_state.solver.are_formulas_sat(guess_assumptions):
            ...
            b = 100
            assert (
                MAX_SIGNED_WORD < a * b <= MAX_UNSIGNED_WORD
            ), "Product should be above max signed word"
            assert (
                int.from_bytes(a.to_bytes(32, "big", signed=False), "big", signed=True)
                > 0
            ), "a should be positive (in signed view)"
            assert (
                int.from_bytes(b.to_bytes(32, "big", signed=False), "big", signed=True)
                > 0
            ), "b should be positive (in signed view)"
            assert (
                int.from_bytes(
                    (a * b).to_bytes(32, "big", signed=False), "big", signed=True
                )
                < 0
            ), "product should be negative (in signed view)"
            signed_overflow_assumption = [
                Equal(operand_0, BVV(a, 256)),
                Equal(operand_1, BVV(b, 256)),
            ]
            signed_overflow_possible = found_state.solver.are_formulas_sat(
                signed_overflow_assumption
            )
        ...
        if found_state.solver.are_formulas_sat(guess_assumptions):
            ...
            dividend, divisor = (
                (operand_0, operand_1) if div_forward else (operand_1, operand_0)
            )
            ...
            div_by_zero_possible = found_state.solver.is_formula_sat(
        ...
        if found_state.solver.are_formulas_sat(guess_assumptions):
            ...
            dividend, divisor = (
                (operand_0, operand_1) if mod_forward else (operand_1, operand_0)
            )
