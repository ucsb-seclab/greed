"""
Contians common functions for both the SAFEMATH's function analysis.
"""
...
def basic_safemath_checks_pass(project: 'Project', function: 'TAC_Function') -> Optional[List[Tuple['TAC_Returnprivate', str]]]:
    """
    ...
    return_blocks: List['Block'] = [block for block in cfg.nodes() if block.statements[-1].__internal_name__ == 'RETURNPRIVATE']
    ...
    for return_block in return_blocks:
        ...
        if len(return_stmt.arg_vars) != 2:
            log.debug(f'Function {function.name} block {return_block.id} does not return exactly one value')
            return None
